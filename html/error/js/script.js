document.addEventListener("DOMContentLoaded", () => {
  ...
  function drawPipes() {
    ...
    pipes.forEach((pipe) => {
      ...
      ctx.fillRect(
        pipe.x,
        pipe.y + pipe.height + pipe.gap,
        pipe.width,
        canvas.height - pipe.y - pipe.height - pipe.gap
      );
    });
  }
    ...
    pipes.forEach((pipe) => {
      if (
        ...
        (bird.y < pipe.y + pipe.height ||
          bird.y + bird.height > pipe.y + pipe.height + pipe.gap)
      ) {
        handleGameOver();
      }
    });
  ...
  function resetGame() {
    ...
    document.getElementById(
      "high-score"
    ).textContent = `Maior Pontuação: ${highScore}`;
  }
